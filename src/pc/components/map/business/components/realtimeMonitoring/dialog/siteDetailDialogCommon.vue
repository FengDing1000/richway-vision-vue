...
<Dialog :title="detailInfo.stnm" @on-hide="handleHide" :config="config">
    <div v-loading="loading" slot="content" class="site-dialog-content">
        <ul>
            <li v-for="(item, index) in currentComponentList" :style="styleObj" :class="[modelConfig.activeId === item.id ? 'active' : '']" :key="index" @click.stop="handleChooseItem(item)">
                <span>{{ item.config.title }}</span>
            </li>
        </ul>
        <div class="site-detail-panel">
            <keep-alive>
                <component :detail-info="detailInfo" :is="modelConfig.activeComponent" :config="modelConfig.activeConfig" />
            </keep-alive>
        </div>
    </div>
</Dialog>
...
export default {
    ...
    computed: {
        ...
        currentComponentList() {
            ...
            selectComponent.getDefaultSelectComponent(list,this.modelConfig);
            return list;
        }
    },
    mounted() {
    ...
    methods: {
        handleChooseItem(item) {
            this._log(item.id);
            selectComponent.handleChooseItem(item,this.modelConfig);
        },
        handleHide() {
            this.$emit(`on-hide`);
        }
